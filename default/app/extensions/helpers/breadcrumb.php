 ...
 * 
 * @package kumbiaphp
 * @author Roberto I Ramirez N <roberto_iran@hotmail.com>
 * @copyright 2010
 * @version 1.0
 * @access public
 */
class Breadcrumb
{
   ...
   * @return void|bool
   */
  public function addCrumb($key = NULL, $title = NULL, $url = NULL)
  {
  ...
  {
    if(!(isset($this->_path['_push_']) && count($this->_path['_push_']) > 0)) return false;
  ...
  {
    if(!(isset($this->_path['_update_']) && count($this->_path['_update_']) > 0)) return false;
    foreach($this->_path['_update_'] as $crumb)
    {
      if(!array_key_exists($crumb['key'], $this->_path)) continue;
      if($crumb['title'] != '.') $this->_path[$crumb['key']]['title'] = $crumb['title'];
      if($crumb['url'] != '.') $this->_path[$crumb['key']]['url'] = $crumb['url'];
    }
  ...
  private function _jobs()
  {
	...
	public function display() {
		...
		// Recorremos items del crumb
		do {
			
			$crumb = current ( $this->_path );
			if (next ( $this->_path ) && ! empty ( $crumb ['title'] )) {
				prev ( $this->_path );
				$html [] = "<li>" . Html::link ( $crumb ['url'], $crumb ['title'] ) . "</li>";
			} elseif (! empty ( $crumb ['title'] )) {
				$html [] = "<li>{$crumb['title']}</li>";
			}
		...
		if (is_null ( $this->attrs ['separator'] )) {
			...
			return "<ul class='{$this->attrs['class_ul']}'>" . implode ( "<li class='{$this->attrs['class_separator']}'><span class=\"divider\">{$this->attrs['separator']}</span></li>", $html ) . '</ul>';
		}		
